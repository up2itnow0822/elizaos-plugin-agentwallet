...
exports.bridgeAction = {
    name: 'WALLET_BRIDGE',
    similes: ['BRIDGE_TOKENS', 'CROSS_CHAIN', 'CCTP_BRIDGE', 'BRIDGE_USDC'],
    description: "Bridge USDC cross-chain via Circle's CCTP V2. Supported routes include " +
    ...
    handler: async (runtime, message, _state, _options, callback) => {
        ...
        try {
            ...
            callback?.({
                text: `Bridging ${params.amount} USDC from ${params.fromChain} → ${params.toChain}. ` +
            ...
            callback?.({ text: `Bridge failed: ${err.message}` });
            return false;
        }
    },
    examples: [
...
function parseBridgeIntent(text) {
    const match = text.match(/bridge\s+([\d.]+)\s+(\w+)\s+from\s+(\w+(?:-\w+)?)\s+to\s+(\w+(?:-\w+)?)/i);
    if (!match)
        return null;
    return {
        amount: match[1],
